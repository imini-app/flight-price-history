...
export async function fetchRouteData(routeKey) {
  let res = await fetch(`/data/${routeKey}.json`);
  if (!res.ok) {
    const parts = routeKey.split('-');
    const reversed = `${parts[1]}-${parts[0]}`;
    res = await fetch(`/data/${reversed}.json`);
  }
...
export function filterPricesByDepartureDate(prices, depDate) {
  return prices
    .filter(p => p.date === depDate && p.snapshot)
    .sort((a, b) => a.snapshot.localeCompare(b.snapshot));
}

export function computeSnapshotStats(snapshotPrices) {
  if (!snapshotPrices || snapshotPrices.length === 0) return null;
  const vals = snapshotPrices.map(p => p.price);
  const first = vals[0];
  ...
  return { first, last, change, changePercent, min, max, avg: Math.round(avg * 10) / 10, median, count: n };
}

export function groupPricesByDeparture(prices) {
  const groups = {};
