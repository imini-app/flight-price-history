...
export async function getRecentChecks({ limit = 10, type } = {}) {
  ...
  if (type && VALID_CHECK_TYPES.includes(type)) {
    sql = `SELECT ip_address, route_key, route_label, travel_date, check_type, created_at FROM \`${TABLE}\` WHERE check_type = ? ORDER BY created_at DESC LIMIT ${n}`;
    params = [type];
  } else {
    sql = `SELECT ip_address, route_key, route_label, travel_date, check_type, created_at FROM \`${TABLE}\` ORDER BY created_at DESC LIMIT ${n}`;
    params = [];
  }
